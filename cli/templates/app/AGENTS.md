...
skies g entity <Module> <Name>     # an always-valid [Entity]
...
- `src/Skies.Framework.Starter.Api` — the backend. `Program.cs` is a thin index; `Modules/<Module>/` holds one bounded
...
- `tests/Skies.Framework.Starter.Tests` — boots the real app and compiles the spec cases under `.specs/*/e2e`, nothing else.
...
skies proof status           # which receipts went stale
...
- The framework ships the skeleton and the enforcement; this app brings its own libraries and business logic in
  plain code. No source generation of behavior, no base classes, no reflection-based discovery.
