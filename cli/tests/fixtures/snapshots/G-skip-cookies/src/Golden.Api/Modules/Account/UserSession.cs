...
/// <summary>A refresh-token slot in a rotating family: the row the framework's <c>RefreshSessions</c> keeps through
/// <see cref="UserSessionStore"/>. <b>Not</b> tenant-scoped: it is a global auth artifact keyed by an unguessable token
/// hash, never part of an org's dataset, so it is looked up directly with no tenant filter to bypass (the tell that it
/// should not be scoped). Only the hash is stored. A slot is opened through <see cref="Start"/> and retired through
/// <see cref="MarkUsed"/>; when a used slot is presented again the framework treats it as theft and burns the family.</summary>
[Entity]
public class UserSession
{
    ...
    /// <summary>The lookup hash of the refresh token; the raw token is never stored.</summary>
    ...
    /// <summary>Open a new slot in <paramref name="familyId"/> for <paramref name="userId"/>, carrying the token's hash
    /// and the expiry the framework's session policy chose. Creation funnels through <see cref="EnsureValid"/>.</summary>
    public static Result<UserSession> Start(Guid userId, Guid familyId, string tokenHash, DateTime now, DateTime expiresAt) =>
        new UserSession
        {
            Id = Guid.NewGuid(),
            UserId = userId,
