...
(`0001-auth#FM-12`). `Logout` revokes the family too (`0001-auth#FM-15`).
...
(`0001-auth#FM-14`). Both lifetimes are this app's policy, set on `AddSkiesAuth`.
...
- **No one grants themselves Admin.** `AppPolicies.AppAdmin`, the policy on writes to app-wide data, needs the
  `Admin` role, and no endpoint here sets a role (`0001-auth#FM-10`).
...
token could not tell "this session" from the others (`0001-auth#FM-17`, `0001-auth#FM-19`,
`0001-auth#FM-20`). The session row is keyed by `UserId` +
