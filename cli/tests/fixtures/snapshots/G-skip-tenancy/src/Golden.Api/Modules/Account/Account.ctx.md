...
(`0001-auth#FM-13`). `Logout` revokes the family too (`0001-auth#FM-16`).
...
(`0001-auth#FM-15`). Both lifetimes are this app's policy, set on `AddSkiesAuth`.
...
- **No one grants themselves Admin.** `AppPolicies.AppAdmin`, the policy on writes to app-wide data, needs the
  `Admin` role, and no endpoint here sets a role (`0001-auth#FM-11`).
...
token could not tell "this session" from the others (`0001-auth#FM-18`, `0001-auth#FM-20`,
`0001-auth#FM-21`). The session row is keyed by `UserId` +
...
client never sees the refresh token in a body (`0001-auth#FM-22`,
`0001-auth#FM-23`).
