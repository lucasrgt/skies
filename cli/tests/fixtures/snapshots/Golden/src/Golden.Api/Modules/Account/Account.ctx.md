...
`TenantStamping` interceptor, which stamps `OrgId` on insert for every save, synchronous or not. So a normal slice
...
(`0001-auth#FM-23`). The one exception is auth-bootstrap — see below.
...
org (`0001-auth#FM-24`). The identity itself — the email, the refresh-token hash — is what
...
- **Registration opens the org.** `Register` opens an `Org` and creates the user in it by name
  (`User.Register(org.Id, …)`), so each sign-up starts in an org of its own (`0001-auth#FM-5`).
...
- **The user is looked up across the filter** (`db.Users.IgnoreQueryFilters()`), and the org is derived
  **from the found user**, then put into the JWT. This forced the identity model: **email is unique
  ...
  org but signs in by email across all of them (`0001-auth#FM-10`).
...
(`0001-auth#FM-7`). `User` stores the hash as the package's opaque `PasswordHash`; to move to
...
client never sees the refresh token in a body (`0001-auth#FM-25`,
`0001-auth#FM-26`).
