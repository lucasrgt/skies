...
public class GoogleSignIn
{
    ...
    [Fact(DisplayName = "FM-7: a request with no id token is denied like a forged one, never a server error")]
    public async Task A_missing_id_token_is_denied()
    {
        await using var app = new TestApp();
        var client = app.CreateClient();

        var register = await client.PostAsJsonAsync("/account/register/google", new { });
        var login = await client.PostAsJsonAsync("/account/login/google", new { });
