...
public class EmailTokens
{
    ...
    [Fact(DisplayName = "FM-8: registering a taken email answers like a new one and mails its owner instead")]
    public async Task A_taken_email_is_told_by_mail_not_by_the_response()
    {
        var mail = new CapturingEmailSender();
        await using var app = new TestApp();
        using var factory = WithMail(app, mail);
        ...
        Assert.Equal(sent + 1, mail.Sent);
        Assert.Equal(Email, mail.LastTo);
        await Login(client, Password);
    }

    [Fact(DisplayName = "FM-9: reset and verification requests missing a field are refused as client errors")]
    public async Task Missing_fields_are_client_errors()
    {
        ...
        var client = factory.CreateClient();
        await Authenticate(client);
        var sentBefore = mail.Sent;

        var requestNoEmail = await client.PostAsJsonAsync("/account/password-reset/request", new { });
        var resetNoPassword = await client.PostAsJsonAsync("/account/password-reset", new { token = "anything" });
        var resetNoToken = await client.PostAsJsonAsync("/account/password-reset", new { newPassword = "new-password1" });
        var verifyNoToken = await client.PostAsJsonAsync("/account/verify-email", new { });

        Assert.True(requestNoEmail.IsSuccessStatusCode, $"reset request answered {requestNoEmail.StatusCode}");
        Assert.Equal(sentBefore, mail.Sent);
        Assert.Equal(HttpStatusCode.BadRequest, resetNoPassword.StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized, resetNoToken.StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized, verifyNoToken.StatusCode);
        await Login(client, Password);
    }
