...
fn generate_package(contract: &Path, output: &Path, name: &str, version: &str) -> Result<bool> {
    ...
    exclude_from_app_analysis(&generated.join("analysis_options.yaml"))?;
...
/// The generated package is verified once, above (an analyzer error fails the run), and then kept out of the app's
/// `flutter analyze`, which also walks `packages/`: dart-dio's output carries warnings nobody can fix (unused and
/// duplicate imports), and the package is replaced as a unit on every run. Excluded files still resolve, so the
/// app's own code is checked against the generated types as before.
fn exclude_from_app_analysis(options: &Path) -> Result<()> {
    std::fs::write(options, GENERATED_ANALYSIS_OPTIONS).with_context(|| format!("writing {}", options.display()))
}

const GENERATED_ANALYSIS_OPTIONS: &str = "\
# Generated by `skies g client`, replaced as a unit on every run: the app's analysis skips this package's own
# files. Their types still resolve, so the code that uses them is checked as usual.
analyzer:
  exclude:
    - \"**\"
";
...
mod tests {
    ...
    #[test]
    fn the_generated_package_is_kept_out_of_the_app_s_analysis() {
        let dir = tempfile::tempdir().unwrap();
        let options = dir.path().join("analysis_options.yaml");
        std::fs::write(&options, "include: package:lints/recommended.yaml\n").unwrap();

        exclude_from_app_analysis(&options).unwrap();

        let text = std::fs::read_to_string(&options).unwrap();
        assert!(text.ends_with("analyzer:\n  exclude:\n    - \"**\"\n"), "{text}");
