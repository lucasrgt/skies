...
pub fn render_seams(package_name: &str, client_class: &str) -> Result<Vec<(&'static str, String)>> {
    Ok(vec![
        (
            ...
            render(
                CLIENT_SEAM,
                context! { package => package_name, class => client_class },
            )?,
        ),
...
fn generate_package(contract: &Path, output: &Path, name: &str) -> Result<bool> {
    ...
    std::fs::write(
        ...
        "generated by skies; replace as a unit\n",
    )?;
...
fn publish(generated: &Path, output: &Path) -> Result<()> {
    if !output.exists() {
        return std::fs::rename(generated, output)
            .with_context(|| format!("moving into {}", output.display()));
    }
...
mod tests {
    ...
    fn pins_dart_dio_built_value_and_package_metadata() {
        let args =
            generator_arguments("contract.json", "client.gen", "sample_api", "1.2.3").unwrap();
        assert_eq!(
            args[..5],
            ["generate", "-g", "dart-dio", "-i", "contract.json"]
        );
        assert!(args.contains(
            &"pubName=sample_api,pubVersion=1.2.3,pubPublishTo=none,serializationLibrary=built_value".into()
        ));
        assert!(
            args.contains(&"apiDocs=false,modelDocs=false,apiTests=false,modelTests=false".into())
        );
    }
    ...
    fn rejects_a_package_name_that_could_inject_generator_properties() {
        ...
        assert!(
            error
                .to_string()
                .contains("lowercase Dart package identifier")
        );
    }
