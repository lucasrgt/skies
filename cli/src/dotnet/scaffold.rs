...
pub(super) const INHERITED_POSTURE: &str = "; // authorization: the module's route group decides";
...
mod tests {
    ...
    fn a_slice_under_an_anonymous_group_is_wired_and_states_no_posture_of_its_own() {
        ...
        assert!(slice.contains(".WithName(nameof(Transfer)); // authorization: the module's route group decides\n"));
    ...
    fn a_fresh_module_gets_a_fail_closed_group_the_slice_inherits() {
        ...
        assert!(slice.ends_with("// authorization: the module's route group decides\n}\n"));
    }
