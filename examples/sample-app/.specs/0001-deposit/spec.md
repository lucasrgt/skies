...
- FM-5 A retry carrying the same `Idempotency-Key` credits the wallet a second time. [avp: idempotency-key-honored]
- FM-6 A new `Idempotency-Key` is treated as a replay and does not credit. [avp: idempotency-key-honored]
