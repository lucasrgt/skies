...
| `SKY0030` | A crossing of the tenant boundary in the app's own code is flagged: `IgnoreQueryFilters` whenever it may lift the tenant filter (no argument, `"tenant"`, a name that is not a literal), and a `FixedTenant` (`new FixedTenant(org)`, `FixedTenant.System`), an org the code picks instead of the request's. Lifting only an app's own named filter (`["soft-delete"]`) stays legal; specs and `*.Tests.cs` are exempt. A deliberate crossing (sign-in by email, a background job) takes `#pragma warning disable SKY0030 // <reason>` or `[SuppressMessage(..., "SKY0030", Justification = "<reason>")]`; a hatch without its reason, or a bare `#pragma warning disable` before a crossing, is itself flagged | a cross-org read or write compiles and looks harmless |
...
`SKY0030` (a tenant crossing) enforces its reason: a `#pragma warning disable SKY0030` with no comment after the
code, a `[SuppressMessage]` of it without a `Justification`, and a bare `#pragma warning disable` before a crossing
are each reported.
